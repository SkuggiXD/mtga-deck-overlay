...
_SPLIT_UP_MODES = {
    0: "destroy all tapped creatures",
    1: "destroy all untapped creatures",
    "0": "destroy all tapped creatures",
    "1": "destroy all untapped creatures",
    "tapped": "destroy all tapped creatures",
    "untapped": "destroy all untapped creatures",
}
...
class MatchRecap:
    ...
    def __init__(self, state: Any, names: Any, match_dir: Path) -> None:
        ...
        self._logged_cast_names: Set[Tuple[Any, ...]] = set()
        self._sweeper: Optional[Dict[str, Any]] = None
    ...
    def _emit_action(self, who: str, verb: str, nm: str, iid: Any) -> bool:
        if not nm or nm == "a card":
            return False
        try:
        ...
        if iid_i is not None and iid_i in self._logged_cast:
            return False
        key = (self._turn_num, verb, nm.lower())
        if key in self._logged_cast_names:
            if iid_i is not None:
                self._logged_cast.add(iid_i)
            return False
        self._logged_cast_names.add(key)
        if iid_i is not None:
            self._logged_cast.add(iid_i)
        self._line(f"  {who} {verb} {nm}")
        if verb == "casts" and "split up" in nm.lower():
            self._sweeper = {"name": nm, "mode": None, "kills": [], "tapped": []}
        return True

    def _maybe_mode(self, details: Dict[str, Any]) -> None:
        if not self._sweeper or self._sweeper.get("mode"):
            return
        for key, val in details.items():
            lk = str(key).lower()
            if not any(w in lk for w in ("mode", "choice", "option", "idoption")):
                blob = str(val).lower()
                if "tapped creature" in blob or "untapped creature" in blob:
                    self._sweeper["mode"] = str(val)
                    return
                continue
            mapped = _SPLIT_UP_MODES.get(val)
            if mapped:
                self._sweeper["mode"] = mapped
                return
            text = str(val)
            if "tapped" in text.lower() or "untapped" in text.lower():
                self._sweeper["mode"] = text

    def _note_kill(self, nm: str, go: Dict[str, Any], cause: str) -> None:
        if not self._sweeper:
            if cause and "split up" in cause.lower():
                self._sweeper = {"name": cause, "mode": None, "kills": [], "tapped": []}
            else:
                return
        if nm and nm != "a card":
            self._sweeper["kills"].append(nm)
            tapped = bool(go.get("tapped") or go.get("isTapped"))
            self._sweeper["tapped"].append(tapped)

    def _flush_sweeper(self) -> None:
        sw = self._sweeper
        self._sweeper = None
        if not sw:
            return
        name = sw.get("name") or "Split Up"
        mode = sw.get("mode")
        kills = sw.get("kills") or []
        flags = sw.get("tapped") or []
        if not mode and flags and len(flags) == len(kills):
            if flags and all(flags):
                mode = "destroy all tapped creatures"
            elif flags and not any(flags):
                mode = "destroy all untapped creatures"
        if mode:
            self._line(f"  {name} mode: {mode}")
        if kills:
            uniq = list(dict.fromkeys(kills))
            self._line(f"  {name} destroys {', '.join(uniq)}")
    ...
    def _annotations(self, anns: Any) -> None:
        ...
        for ann in anns:
            ...
            if any("ResolutionStart" in t or "ResolutionComplete" in t for t in types):
                ...
                if "Complete" in "".join(types) and nm and "split up" in nm.lower():
                    self._flush_sweeper()
    ...
    def _zone_transfer(self, details: Dict[str, Any], affector: Any, affected: List[Any]) -> None:
        ...
        for iid in iids:
            ...
            if cat == "CastSpell" or (dest_t == "ZoneType_Stack" and cat in ("", "Cast", "Play", "ActivateAbility", "Trigger", "TriggeredAbility")):
                ...
                self._emit_action(who, verb, nm, iid_i)
                continue
            elif cat == "PlayLand":
                ...
                if dest_t == "ZoneType_Graveyard":
                    self._note_kill(nm, go, cause)
                ...
                # Sweeper summary prints the kill list; skip per-card die lines
                # while Split Up is resolving so the recap stays readable.
                if dest_t == "ZoneType_Graveyard" and self._sweeper and (
                    not cause or "split up" in cause.lower() or "split up" in (self._sweeper.get("name") or "").lower()
                ):
                    continue
                ...
                kinds = [k.lower() for k in _type_words(go)]
                already = (self._turn_num, "casts", (nm or "").lower()) in self._logged_cast_names
                if already and any(k in kinds for k in ("aura", "enchantment", "equipment")):
                    continue
                ...
                self._emit_action(who, "casts" if not ability else "triggers", nm, iid)
    ...
    def _stack_casts(self) -> None:
        ...
        for iid in now - self._stack_seen:
            ...
            self._emit_action(self._who(seat), verb, nm, iid)
