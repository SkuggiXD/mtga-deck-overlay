...
def parse_deck_text(text: str, names: CardNames) -> Tuple[Counter, str]:
    """Accepts Arena/MTGO-style text lists. Uses negative ids so they do not collide with grpIds."""
    counts: Counter = Counter()
    alias: Dict[int, str] = {}
    next_id = -1
    ...
    with names._lock:
        for iid, nm in alias.items():
            if nm and not _looks_like_placeholder(nm, iid):
                names.ids[str(int(iid))] = nm
    names.save()
